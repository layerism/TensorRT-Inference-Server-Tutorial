...
def get_shape(m: onnx.ModelProto, name: str) -> TensorShape:
    """
    Note: This method relies on onnx shape inference,
    which is not reliable. So only use it on input or output tensors
    """
                ...
                [
                    onnx.ValueInfoProto(
                        ...
                        type=onnx.TypeProto(
                            ...
                            Tensor(elem_type=x.data_type, shape=shape)
                        )
                    )
                ]
            )
    ...
    for key in full_input_shapes:
        if np.prod(full_input_shapes[key]) <= 0:
            raise RuntimeError(
                ...
                'please determine the input size manually by --input-shape xxx'.
                format(key)
            )
...
def forward_all(model: onnx.ModelProto,
                input_shapes: TensorShapes = {}) -> Dict[str, np.ndarray]:
    import copy
...
def eliminate_const_nodes(
    model: onnx.ModelProto, const_nodes: List[onnx.NodeProto], res: Dict[str, np.ndarray]
) -> onnx.ModelProto:
    """
    ...
    for i in range(n_times):
        ...
        for name in res_opt.keys():
            if not np.allclose(res_opt[name], res_ori[name], rtol=1e-4, atol=1e-5):
                print(
                    "Tensor {} changes after simplifying. The max diff is {}.".format(
...
def check_and_update_input_shapes(
    model: onnx.ModelProto, input_shapes: TensorShapes
) -> TensorShapes:
    input_names = get_input_names(model)
